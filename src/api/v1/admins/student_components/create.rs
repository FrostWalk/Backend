...
#[utoipa::path(
    ...
    responses(
        (status = 200, description = "Student component created successfully", body = CreateStudentComponentResponse),
...
pub(super) async fn create_student_component_handler(
    ...
    let existing = StudentsComponent::where_col(|sc| sc.project_id.equal(scheme.project_id))
    ...
    let mut state = DbState::new_uncreated(StudentsComponent {
        students_component_id: 0,
        project_id: scheme.project_id,
        name: scheme.name.clone(),
    });
    ...
    Ok(HttpResponse::Ok().json(CreateStudentComponentResponse {
        students_component_id: state.students_component_id,
        project_id: scheme.project_id,
        name: scheme.name,
    }))
}
