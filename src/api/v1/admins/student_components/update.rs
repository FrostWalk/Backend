...
#[utoipa::path(
    ...
    responses(
        (status = 200, description = "Student component updated successfully"),
...
pub(super) async fn update_student_component_handler(
    ...
    let mut rows = StudentsComponent::where_col(|sc| sc.students_component_id.equal(id))
        .run(&data.db)
        .await
        .map_err(|e| {
    ...
    let existing =
        StudentsComponent::where_col(|sc| sc.project_id.equal(component_state.project_id))
            .where_col(|sc| sc.name.equal(&scheme.name))
            .where_col(|sc| sc.students_component_id.not_equal(id))
            .run(&data.db)
            .await
            .map_err(|e| {
                error_with_log_id_and_payload(
