...
pub fn check_field_type_derive(input: TokenStream) -> TokenStream {
    ...
    let fields = if let Data::Struct(data_struct) = &input.data {
        if let Fields::Named(fields_named) = &data_struct.fields {
            ...
            return syn::Error::new_spanned(struct_name, "Only structs with named fields are supported").to_compile_error().into();
        }
    } else {
    ...
    let field = fields.iter().find(|field| field.ident.as_ref().unwrap() == FIELD_NAME);

    match field {
        // Field is not present
        None => {
            return syn::Error::new_spanned(struct_name, format!("The struct must have a field `{}` of type `{}`", FIELD_NAME, FIELD_TYPE))
                .to_compile_error()
                .into();
        }
        ...
        Some(f) => {
            if let Type::Path(type_path) = &f.ty {
                if !type_path.path.is_ident(FIELD_TYPE) {
                    let span = &f.ty.span();
                    return syn::Error::new(*span, format!("Expected: `{}` but found: `{}`", FIELD_TYPE, &f.ty.to_token_stream())).to_compile_error().into();
                }
            }
        }
    }
